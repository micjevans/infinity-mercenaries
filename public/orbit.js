...
(() => {
  ...
  var OrbitBase = class extends HTMLElement {
    ...
    getCommonAttributes(element) {
      const orbitRadius = parseFloat(
        getComputedStyle(element).getPropertyValue("r") || 0,
      );
      const orbitNumber = parseFloat(
        getComputedStyle(element).getPropertyValue("--o-orbit-number") || 1,
      );
      const size = parseFloat(
        getComputedStyle(element).getPropertyValue("--o-size-ratio") || 1,
      );
      const strokeWidth = parseFloat(
        getComputedStyle(element).getPropertyValue("--o-stroke-width") || 1,
      );
    ...
    calculateCommonArcParameters(
      ...
      const smallRadius =
        radius - arcHeightPercentage !== 0
          ? radius - arcHeightPercentage
          : radius;
    ...
    generateRoundedPath(params, arcHeight, orbitNumber) {
      ...
      const newUpperStart = this.arcPoint(
        bigRadius,
        params.upperAngleStart,
        0,
        curve / orbitNumber,
      );
      const newUpperEnd = this.arcPoint(
        bigRadius,
        params.upperAngleEnd,
        0,
        -curve / orbitNumber,
      );
      const newInnerStart = this.arcPoint(
        smallRadius,
        params.innerAngleStart,
        0,
        curve / orbitNumber,
      );
      const newInnerEnd = this.arcPoint(
        smallRadius,
        params.innerAngleEnd,
        0,
        -curve / orbitNumber,
      );
      const upperPointStart = this.arcPoint(
        bigRadius,
        params.upperAngleStart,
        -(curve / 2) / orbitNumber,
        0,
      );
      const upperPointEnd = this.arcPoint(
        bigRadius,
        params.upperAngleEnd,
        -(curve / 2) / orbitNumber,
        0,
      );
      const innerPointStart = this.arcPoint(
        smallRadius,
        params.innerAngleStart,
        curve / 2 / orbitNumber,
        0,
      );
      const innerPointEnd = this.arcPoint(
        smallRadius,
        params.innerAngleEnd,
        curve / 2 / orbitNumber,
        0,
      );
      const Q = this.getControlPoint(
        newUpperEnd.x,
        newUpperEnd.y,
        upperPointEnd.x,
        upperPointEnd.y,
      );
      const Q1 = this.getControlPoint(
        innerPointEnd.x,
        innerPointEnd.y,
        newInnerEnd.x,
        newInnerEnd.y,
      );
      const Q2 = this.getControlPoint(
        newInnerStart.x,
        newInnerStart.y,
        innerPointStart.x,
        innerPointStart.y,
      );
      const Q3 = this.getControlPoint(
        upperPointStart.x,
        upperPointStart.y,
        newUpperStart.x,
        newUpperStart.y,
    ...
    generateArrowPath(params, orbitNumber) {
      ...
      const middleEnd = this.arcPoint(
        radius,
        params.upperAngleEnd,
        0,
        24 / orbitNumber / 2,
      );
      const middleStart = this.arcPoint(
        radius,
        params.upperAngleStart,
        0,
        24 / orbitNumber / 2,
    ...
    generateSlashPath(params, shape, orbitNumber) {
      ...
      const newUpperStart = this.arcPoint(
        ...
        upperAngleStart,
        0,
        shape === "backslash" ? 0 : 24 / orbitNumber / 2,
      );
      const newUpperEnd = this.arcPoint(
        bigRadius,
        upperAngleEnd,
      ...
      const newInnerStart = this.arcPoint(
        ...
        innerAngleStart,
        0,
        shape === "backslash" ? 24 / orbitNumber / 2 : 0,
      );
      const newInnerEnd = this.arcPoint(
        smallRadius,
        innerAngleEnd,
    ...
    generateZigzagPath(params, arcHeight, orbitNumber) {
      ...
      const s2 = this.arcPoint(
        radius,
        params.upperAngleStart,
        -arcHeight / orbitNumber / 2,
        3,
      );
      const s3 = this.arcPoint(
        radius,
        params.upperAngleStart,
        0 / orbitNumber / 2,
        0,
      );
      const s4 = this.arcPoint(
        radius,
        params.upperAngleStart,
        arcHeight / orbitNumber / 2,
        3,
      );
      const e2 = this.arcPoint(
        radius,
        params.innerAngleEnd,
        arcHeight / orbitNumber / 2,
        3,
      );
      const e3 = this.arcPoint(
        radius,
        params.innerAngleEnd,
        0 / orbitNumber / 2,
        0,
      );
      const e4 = this.arcPoint(
        radius,
        params.innerAngleEnd,
        -arcHeight / orbitNumber / 2,
        3,
  ...
  <style>
    ...
    text {
      fill: var(--o-color);
      font-size: var(--o-fixed-font-size, inherit) !important;
      text-anchor: middle;
...
`;
  var OrbitArc = class extends OrbitBase {
    ...
    disconnectedCallback() {
      ...
      (_b = this.textObserver) == null ? void 0 : _b.disconnect();
    }
    setupObservers() {
      this.observer = new MutationObserver((mutations) => {
        this.observer.disconnect();
    ...
    update() {
      ...
      if (fitRange) {
        textPath.parentElement.setAttribute(
          "textLength",
          orbitPath.getTotalLength(),
        );
      }
      const fixedFontSize = getComputedStyle(this).getPropertyValue(
        "--o-fixed-font-size",
      );
      if (fixedFontSize) {
        text.style.fontSize = fixedFontSize.trim();
      } else {
    ...
    getAttributes() {
      ...
      const gap = parseFloat(
        getComputedStyle(this).getPropertyValue("--o-gap"),
      );
      if (value) {
    ...
    calculateArcParameters() {
      ...
      const dShape = super.generatePathData(
        shape,
        params,
        arcHeight,
        orbitNumber,
    ...
    calculateTextArcParameters() {
      ...
      const largeArcFlag = arcAngle <= 180 ? 0 : 1;
      let coordX1 =
        50 + realRadius * Math.cos((-90 + adjustedGap) * (Math.PI / 180));
      let coordY1 =
        50 + realRadius * Math.sin((-90 + adjustedGap) * (Math.PI / 180));
      let coordX2 =
        50 +
        realRadius * Math.cos(((arcAngle - 90 - adjustedGap) * Math.PI) / 180);
      let coordY2 =
        50 +
        realRadius * Math.sin(((arcAngle - 90 - adjustedGap) * Math.PI) / 180);
        ...
        for (let entry of entries) {
          ...
          if (childElements) {
            childElements.forEach((childElement) => {
              let gravityForce =
                getComputedStyle(childElement).getPropertyValue("--o-force");
